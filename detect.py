...
def detect(img, detector, encoder, encoding_dict, attendance_tracker):
    ...
    for res in results:
        ...
        current_time = datetime.now().timestamp()  # Convert to timestamp
        for student_name, last_seen_time in attendance_tracker.last_seen.items():
            if student_name not in present_students and last_seen_time is not None and current_time - last_seen_time <= 600:  # 10 minutes in seconds
...
if __name__ == "__main__":
    ...
    while cap.isOpened():
        ...
        # Check if it's time to update the spreadsheet
