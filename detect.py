...
def write_attendance_to_file(attendance_tracker, timestamps):
    ...
    for timestamp in timestamps:
        ...
        if not os.path.exists(file_path):  # If file doesn't exist, create it and write header
            with pd.ExcelWriter(file_path) as writer:
                df.to_excel(writer, index=False, sheet_name='Attendance', startrow=0)
        else:  # If file exists, append data without header
            with pd.ExcelWriter(file_path, mode='a') as writer:
                df.to_excel(writer, index=False, sheet_name='Attendance', startrow=None, header=None)
...
def detect_faces(detector, img_rgb, results_queue):
    ...
    Args:
        ...
        results_queue: Queue to store the detection results
    """
...
def process_frame(frame, face_detector, face_encoder, encoding_dict, attendance_tracker, last_seen_faces):
    ...
    for i, res in enumerate(results):  # Use enumerate to loop over the results
        ...
        for db_name, db_encode in encoding_dict.items():
            ...
            if dist <= recognition_t and dist < distance:  # Check if distance is less than or equal to recognition_t
                name = db_name
                distance = dist
