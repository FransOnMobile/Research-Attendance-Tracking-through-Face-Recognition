...
for face_names in os.listdir(face_data):
    ...
    for image_name in tqdm(os.listdir(person_dir), desc=f'Training {face_names}', total=total_images, leave=True):
    ...
    if encodes:
        encode = np.mean(encodes, axis=0)  # Take the mean encoding
        encode = l2_normalizer.transform(np.expand_dims(encode, axis=0))[0]
        encoding_dict[face_names] = encode

    encodes = []  # Reset encodes for the next person
